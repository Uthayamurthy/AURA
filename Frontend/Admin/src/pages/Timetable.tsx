...
export default function Timetable() {
    ...
    const uploadMutation = useMutation({
        mutationFn: async () => {
            ...
            await api.post('/admin/timetable/bulk_upload', form, {
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
                        ...
                        <DialogContent>
                            <DialogHeader><DialogTitle>Import Timetable</DialogTitle></DialogHeader>
                            <div className="py-4 space-y-2">
                                <p className="text-sm text-gray-500">Columns: Class Name, Day (Name/0-5), Period (1-8), Course Code, Prof Email</p>
                    ...
                    <Select value={selectedClass} onValueChange={setSelectedClass}>
                        <SelectTrigger className="w-[200px]"><SelectValue placeholder="Select Class Group" /></SelectTrigger>
                        <SelectContent>
                        ...
                        <div className="flex border-b">
                            ...
                            {bellSchedule.map((slot: any) => (
                                <div key={slot.slot_number} className="flex-1 p-2 text-center border-r last:border-r-0 min-w-[100px]">
                                    ...
                                    <div className="text-xs text-gray-500">{slot.start_time?.slice(0, 5)} - {slot.end_time?.slice(0, 5)}</div>
                                </div>
                            ))}
                        </div>
                        {DAYS.map((day, dayIdx) => (
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            ...
                            <Select value={assignForm.courseId} onValueChange={v => setAssignForm({ ...assignForm, courseId: v })}>
                                ...
                                <SelectContent>{courses.map((c: any) => <SelectItem key={c.id} value={c.id.toString()}>{c.code}: {c.name}</SelectItem>)}</SelectContent>
                            </Select>
                        </div>
                        <div className="space-y-2">
                            ...
                            <Select value={assignForm.profId} onValueChange={v => setAssignForm({ ...assignForm, profId: v })}>
                                ...
                                <SelectContent>{professors.map((p: any) => <SelectItem key={p.id} value={p.id.toString()}>{p.name}</SelectItem>)}</SelectContent>
                            </Select>
                        </div>
            ...
            <Dialog open={isBellOpen} onOpenChange={setIsBellOpen}>
                <DialogContent className="max-w-xl">
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={() => updateBellScheduleMutation.mutate()} disabled={updateBellScheduleMutation.isPending}>Save Schedule</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
